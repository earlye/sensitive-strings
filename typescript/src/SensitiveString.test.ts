...
// License: MIT
import { SensitiveString, type JSONReplacer } from "./SensitiveString.ts";
...
test("[af5a2178] - SensitiveString allows access to value", () => {
  ...
  assert.equal(JSON.stringify(obj, SensitiveString.PlaintextReplacer((key:string, value:any) => value) as JSONReplacer), JSON.stringify({ ss: "foo", "b": "c" }));
...
test("[0E568C2C] - Object.prototype.toString should not leak raw value", () => {
  const ss = new SensitiveString("secretToStringTag");

  const result = Object.prototype.toString.call(ss);
  console.log("Object.prototype.toString.call(ss):", result);

  assert(!result.includes("secretToStringTag"), "Object.prototype.toString leaked raw value!");
  assert(result === "[object Object]" || result === "[object SensitiveString]",
    "Object.prototype.toString should return [object Object] or [object SensitiveString]");
});
